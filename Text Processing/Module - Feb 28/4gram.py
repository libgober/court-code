...
def analyzer(doc):
    ...
    list_of_quadrograms = izip(list_of_words, islice(list_of_words,1,None), islice(list_of_words,2,None),islice(list_of_words,3,None))
    
    #These will be tuples which is inconvenient for writing to csv since we'll get separate columns
    better_list_of_quadrograms  = [tup[0] + " " + tup[1] + " " + tup[2] + " " + tup[3] for tup in list_of_quadrograms]
    
    #count the bigrams
    tabulation = Counter(better_list_of_quadrograms).items()
